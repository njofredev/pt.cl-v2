...
const AccessibilityIcon = ({ className, size = 22 }: { className?: string; size?: number }) => (
  <svg
  ...
  return (
    <div className="relative inline-block" ref={menuRef}>
      ...
      <button
        ...
        className={`w-10 h-10 md:w-9 md:h-9 rounded-full flex items-center justify-center border transition-all hover:scale-105 active:scale-95 cursor-pointer shadow-md bg-transparent border-transparent ${isOpen
            ? 'text-[#259CF4]'
            : 'text-[#154F92]'
          }`}
      >
        <AccessibilityIcon className={`w-full h-full ${isOpen ? 'animate-pulse' : ''}`} />
      </button>
          ...
          >
            ...
            <div className="bg-[#259CF4]/10 dark:bg-slate-800/50 px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
              <div className="flex items-center gap-2">
                <AccessibilityIcon size={20} className="text-[#259CF4]" />
